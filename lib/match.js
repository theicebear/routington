  ...
  while (length) {
    ...
    for (var i = 0, l = nodes.length; i < l; i++) {
      ...
      if (!(regex = node.regex) || regex.test(frag)) {
        ...
        // try to match all the rest of url
        // if there is no more child nodes in current node
        if (Object.keys(node.child).length === 0 && node.children.length === 0) {
          frag += ('/' + decode(frags.join('/')));
          if (!(regex = node.regex) || regex.test(frag)) {
            if (name = node.name) match.param[name] = frag
